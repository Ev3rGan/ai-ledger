...
| Administrative operator | 检查 exact plan 并执行一次显式批准；已退役的直接 Story review/publish 命令不属于支持流程 |
...
当前辅助编排流是：`eligible unreviewed Stories → Editorial Agent → persisted immutable Digest Plan → operator inspects exact version → optional immutable Story-removal derivation(s) → one explicit approval → immutable Editorial Outcome`。每次移除都会生成带 predecessor、Story key、原因和 actor 审计信息的新版本，不修改旧计划，也不会再次调用 Provider。批准事务只接受最新计划固定的内容：零 Story Plan 形成成功的 no-publication 终态，不创建 Digest、RSS 条目或索引任务；非空 Plan 接受 included Stories、发布 unchanged Digest，并在同一事务中排入完整检索索引跟进。计划支持 0–12 条 Story；少于三个 Publisher 会显示非阻断告警。Agent 不自动发布，不逐条申请零散批准，也不输出或持久化 hidden reasoning。该边界记录在 [ADR 0007](../adr/0007-editorial-approval-boundary.md) 与 [ADR 0011](../adr/0011-zero-story-outcomes-and-index-follow-up.md)。
...
- [`persistence.py`](../../src/ai_intel_agent/persistence.py)：不可变 plan/outcome records，以及绑定 exact plan 的原子 `approve_digest_plan` transaction 与持久索引跟进状态。
- [`index_followup.py`](../../src/ai_intel_agent/index_followup.py)：由既有 Scheduler 调用的完整索引重建、合并与失败落库边界。
- [`cli.py`](../../src/ai_intel_agent/cli.py)：只读 `story list/show` 与受支持的 `digest plan prepare/show/remove/approve/follow-up-status/follow-up-retry` operator 子组；直接审核/发布命令是待后续依赖证明后删除的兼容入口。
- [ADR 0007](../adr/0007-editorial-approval-boundary.md)：当前“一份计划、一次显式批准”的决策记录。
- [ADR 0011](../adr/0011-zero-story-outcomes-and-index-follow-up.md)：零 Story 终态与发布后索引跟进的决策记录。
...
uv run ai-intel-agent digest plan follow-up-status --help
uv run ai-intel-agent digest plan follow-up-retry --help
uv run --extra dev pytest tests/test_m3_digest.py -q
```

真正执行 exact plan 批准前，必须按[本地 runbook](../mvp-local-runbook.md)确认使用同一 commit 与隔离数据库。生产操作只遵循[生产 runbook](../mvp-production-runbook.md)和独立授权。
