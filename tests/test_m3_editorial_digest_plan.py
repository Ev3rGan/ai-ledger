...
from editorial_test_support import (
    FakeEditorialProvider as _FakeEditorialProvider,
)
from editorial_test_support import (
    persist_pending_stories as _persist_pending_stories,
)
    ...
    try:
        ...
        try:
            with Session(engine) as session:
                ...
                assert session.scalar(text("SELECT version_num FROM alembic_version")) == "0017"
                assert (
                    session.scalar(
                        text(
